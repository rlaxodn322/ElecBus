...
const BusRouteMap: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const responseStations = await axios.get('http://localhost:3000/api/stations2');
        ...
        const responseBuses = await axios.get('http://localhost:3000/api/bus2');
    ...
    // 주기적으로 버스 위치 업데이트
    const intervalId = setInterval(fetchData, 1000000); // 5초마다 업데이트
  ...
  const busImageSrc = '/icons/icons/bus-move2.svg';
